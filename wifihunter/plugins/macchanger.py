...
class Macchanger(Dependency):
    ...
    def down_macch_up(cls, iface, options):
        ...
        Color.p(
            '\r{+} {C}macchanger{W}: taking interface {C}%s{W} down...' % iface)

        Ifconfig.down(iface)

        Color.clear_entire_line()
        Color.p(
            '\r{+} {C}macchanger{W}: changing mac address of interface {C}%s{W}...' % iface)
        ...
        Color.clear_entire_line()
        Color.p(
            '\r{+} {C}macchanger{W}: bringing interface {C}%s{W} up...' % iface)
    ...
    def reset(cls):
        ...
        if cls.down_macch_up(iface, ['-p']):
            ...
            Color.clear_entire_line()
            Color.pl(
                '\r{+} {C}macchanger{W}: reset mac address back to {C}%s{W} on {C}%s{W}' % (new_mac, iface))
    ...
    def random(cls):
        ...
        if cls.down_macch_up(iface, ['-e']):
            ...
            Color.clear_entire_line()
            Color.pl(
                '\r{+} {C}macchanger{W}: changed mac address to {C}%s{W} on {C}%s{W}' % (new_mac, iface))
